...
struct node* Traverse_ShuangXiang_ListLink(struct node* head)
{
	...
	do{
		...
		p=p->next;
	}while(p!=NULL);//在双向链表中,如果需要指针跑到最后一个节点,
	                //一般会使用
...
struct node* insert_ShuangXiang_ListLink(struct node* head,struct node* p)
{
	...
	else //非空链表,至少有一个结点
	{
		...
		{
			while(p->num > p1->num && p1->next != NULL) //必须是p1->next != head,不能是p1 != head
			{
				current=p1;
				p1=p1->next;
			}
			if(p->num < p1->num)//通过以上while循环可知道,无论如何,p1这个遍历节点都会在需要插入节点的右边. p1跑到p的位置发现p->num < p1->num(这是必然的)
			{
				...
				else    //结点p在链表中间插入,如果p1不是头结点,那么此时插入结点p小于p1,就要让current指向p,而p指向p1
				{   
				    current->next=p;
				    p->previous=current;  
				    p->next=p1;
